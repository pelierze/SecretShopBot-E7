...
class EquipmentRerollBot:
    ...
    def __init__(
        ...
        self.option_panel_bounds = self._normalize_option_panel_bounds(option_panel_bounds)
    ...
    def _normalize_option_panel_bounds(self, option_panel_bounds: Optional[Dict[str, float]]) -> Dict[str, float]:
        if not option_panel_bounds:
            return dict(self.OPTION_PANEL_BOUNDS)

        normalized = dict(self.OPTION_PANEL_BOUNDS)
        try:
            for key in ("left", "top", "right", "bottom"):
                if key in option_panel_bounds:
                    normalized[key] = float(option_panel_bounds[key])
        except (TypeError, ValueError):
            logger.warning("잘못된 옵션 범위 좌표가 전달되어 기본값을 사용합니다: %s", option_panel_bounds)
            return dict(self.OPTION_PANEL_BOUNDS)

        normalized["left"] = min(max(normalized["left"], 0.0), 1.0)
        normalized["top"] = min(max(normalized["top"], 0.0), 1.0)
        normalized["right"] = min(max(normalized["right"], 0.0), 1.0)
        normalized["bottom"] = min(max(normalized["bottom"], 0.0), 1.0)
        if normalized["left"] >= normalized["right"] or normalized["top"] >= normalized["bottom"]:
            logger.warning("옵션 범위 좌표 순서가 잘못되어 기본값을 사용합니다: %s", option_panel_bounds)
            return dict(self.OPTION_PANEL_BOUNDS)
        return normalized
